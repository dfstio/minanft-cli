...
async function verifyPNG(name, key, png) {
    try {
        if ((0, debug_1.debug)())
            console.log("Verifying NFT png:", { name, key });
        ...
        if (pngProof === undefined)
            throw new Error(`Proof ${name} is empty`);
        if (pngProof.originalRoot === undefined)
            throw new Error(`Proof ${name} is not valid`);
        const hash = o1js_1.Poseidon.hash([
            minanft_1.MinaNFT.stringToField(key),
            o1js_1.Field.fromJSON(files[0].value.data),
            minanft_1.MinaNFT.stringToField("file"),
        ]);
        if (hash.toJSON() !== files[0].proof.publicInput[4])
            throw new Error(`Proof ${name} is not valid`);
        const checkPNG = await (0, redactedproof_1.verifyRedactedPNGProofJSON)(pngProof, name + "." + key, png);
        if (!checkPNG)
